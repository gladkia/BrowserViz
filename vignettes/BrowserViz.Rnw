...
and can be used in your customized web (Javascript, HTML, CSS) file/s like this (see \Biocpkg{BrowserVizDemo} for the full story).

\begin{verbatim}

   hub = BrowserViz();
   bvDemo = BrowserVizDemo()      // create your own Javascript module object
   bvDemo.init(hub);              // initialization of your webapp
   hub.addOnDocumentReadyFunction(bvDemo.initializUI);  // register functions the hub will call

      // register your web apps message handlers: the hub will 
      // dispatch incoming messages to these 
   ...
   hub.start();                   // and started
      ...
      // the hub make that easy
      // first, build up your four-field JSON message, then
   hub.send(return_msg);
  
\end{verbatim}

there is unfortunately no alternative to copying the 
