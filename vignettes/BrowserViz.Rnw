...
a simple x-y plotter (see \Biocpkg{BrowserPlot2D}) to illustrate how to write a BrowserViz subclass
...
<<RJSON>>=
  library(jsonlite)
  msg <- toJSON(list(cmd="setWindowTitle", status="request", 
...
\begin{itemize}
   ...
   \item \emph{close}:
   \item \emph{send}:
   \item \emph{getWindowSize}:
   \item \emph{getWindowTitle}:
   \item \emph{setWindowTitle}:
\end{itemize}
...
name is provided to the BrowserViz constructor.  We depend upon the \Biocpkg{httpuv} package which
functions primarily as a websocket server, but also includes a simple http web server.  Once the
web socket is created and opened by \Biocpkg{BrowserViz}, the constructor asks the default 
web browser to display the web component html page.  Javascript in this page then opens a connection
back to the \Biocpkg{BrowserViz} web socket server, and the application is ready to go.

\section{Simple Subclassing Overview}

The \Biocpkg{BrowserPlot2D} package shows how to build upon \Biocpkg{BrowserViz}.  \Biocpkg{BrowserPlot2D} 
...
the web component of a new visualization application, there is unfortunately no alternative to copying the 
\Biocpkg{BrowserViz} \emph{inst/scripts/viz.html} file into your new package's \emph{inst/scripts} 
directory, preserving everything that is of use, then adding your own HTML markup, CSS, Javascript
and probably jQuery, Boostrap and d3.
